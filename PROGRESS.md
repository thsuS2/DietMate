...
- [ ] **반복 거래 등록**
  ...
  - 반복 패턴 설정 (매월/매주/매일)
  - 자동 거래 생성 로직
  - ✅ Zustand 스토어 템플릿/자동 생성 로직 구현 (1차 UI 연동 완료)
  - ✅ 반복 거래 관리 화면/네비게이션 구현 (2025-11-13, 테스트/QA 보완 예정)
...
- 세부 규칙과 최신 지침은 항상 `AIRule.md`를 확인하고 필요 시 업데이트합니다.
...
### 우선순위 1: 가계부 고도화
1. **반복 거래 기능 고도화** (banking.md 2번째 미구현)
   - 반복 거래 템플릿 검증 및 QA 케이스 정리
   - 자동 생성 피드백/알림 UX 보완
   - 관리 화면 세부 편의 기능 확장
